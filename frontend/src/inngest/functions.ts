    ...
    const { userId, credits, endpoint, body } = await step.run(
      ...
      async () => {
        ...
        if (song.fullDescribedSong) {
          endpoint = env.SIMPLE_MODE;
          body = {
            song_description: song.fullDescribedSong,
            ...commomParams,
          };
        }
        ...
        // Custom mode: Lyrics + prompt
          ...
          endpoint = env.CUSTOM_MODE_MANUAL_LYRIC;
          body = {
            lyrics: song.lyrics,
            prompt: song.prompt,
            ...commomParams,
        ...
        // Custom mode: Prompt + described lyrics
          ...
          endpoint = env.CUSTOM_MODE_AUTO_LYRIC;
          body = {
            lyrics_description: song.describedLyrics,
            prompt: song.prompt,
            ...commomParams,
          };
        }
    ...
    if (credits > 0) {
      ...
      await step.run("update-song-result", async () => {
        ...
        await db.song.update({
          ...
          data: {
            s3Key: responseData?.audio_s3_key,
            thumbnails3Key: responseData?.thumbnail_s3_key,
